...
## Dense MLP matches the fused kernel, measured 2026-09-23

A separate `gate_proj` and `up_proj` CUTLASS call on `gx10-23ec` matches a fused `gate_up` call at bf16 neq 0. The 1.8% FFN gap was the SwiGLU store. The official dense MLP runs `silu_and_mul_with_clamp` in bf16: clamp, round SiLU to bf16, then round the product to bf16. An fp32 product of the same gate and up reproduces the shipped CPU FFN (relative error `0.018330`, 3692 lanes). The bf16 store matches the captured official FFN at bf16 neq 0. CUTLASS also multiplies `input_scale * weight_scale_2` once after the dot. Putting both scales inside the sum leaves 3 bf16 lanes on `gate_proj`. `weight_scale_2` is still a multiply.

The fused mHC pre dots the fp32 post output and collapses the bf16 residual. Doing that, on the same worker and revision `09b04e5e74bca08ca8549fc736d4cdd8624bfde3`, prompt `13041`, temperature 0, `new_tokens=2`, one greedy printed `token_ids=13041,13041`, committed length 2. Step 1 argmax was `13041` at logit `14.2659`, runner-up `22654` at `12.9324`, and `198` at `8.3591`. Step 2 argmax was `13041` at `16.6455`, and `198` at `9.67465`. The independent reference is still `13041,198`. Host `nvfp4_decode` and `text_forward` passed. The full 12-test suite was not rerun. TP=2 and DFlash2 were not rerun.

Layer 0 prompt step versus the official dumps: attention input neq 0, attention output neq 7, MLP input neq 3 (relative `0.000026`), FFN neq 0. The same step's generated-token FFN is also neq 0. Layer 1 attention input differs by 14 bf16 lanes (relative `0.000195`). Layer 1 attention output relative error is `0.009508`, and its FFN relative error is `0.070454`. Layer 4's routed set matches the official set `11,67,195,194,100,253,106,86`. Layer 3 still has expert `74` where the official set has `23`. Route set agreement over layers 3–44 was 8. An eight-way fp32 mHC reduction and a 1024-lane norm sum were tried and taken back out: the eight-way reduction moved the first id to `22654`, and the lane sum did not remove the 14 layer-1 lanes.
