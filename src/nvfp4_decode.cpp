...
Nvfp4MatrixView::Nvfp4MatrixView(std::span<const std::uint8_t> packed,
    ...
    if (!std::isfinite(global_scale) || global_scale < std::numeric_limits<float>::min())
        throw std::invalid_argument("NVFP4 global scale must be finite and at least the minimum normal float");
...
float Nvfp4MatrixView::at(std::size_t row, std::size_t col) const {
    ...
    // Double intermediate avoids a spurious overflow. ModelOpt applies
    // weight_scale_2 by multiplication, not division.
